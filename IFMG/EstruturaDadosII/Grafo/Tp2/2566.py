...
class Grafo():
    ...
    def Dijkstra(grafo, verticeOrigem):
        ...
        def menorRelativo(vetor, numeroReferencia):
            ...
            #Caso não chegue até ele retorna -1
            except:
                return -1
        ...
        for nPasso in range(1, grafo.qtdVertice+1):
            ...
            #Se retornou codigo de erro é porque apartir deste ponto o grafo é disconexo
            if(celulaMenorRelativo == -1):
                break
...
while(1):
    try:
        #Entrada
        cabecalho = input().split(" ") #Quantidade de Sedes
        N = int(cabecalho[0]) #Numero de cidades e a cidade BH
        M = int(cabecalho[1]) #Numero de trechos
        ...
        for i in range(M):
            ...
            #Feito por onibus
            if(tipo == 0):
                onibus.append( (origen, destino, valor) )
            #Feito por aviao
            if(tipo == 1):
                aviao.append( (origen, destino, valor) )


        #Monta um grafo para caminho Terrestre e umpara caminho aereo
        grafoTerrestre = Grafo(1)
        grafoTerrestre.listaAdjacencia(N, aviao, direcional = direcional)

        grafoAereo = Grafo(1)
        grafoAereo.listaAdjacencia(N, onibus, direcional = direcional)

        caminhoTerrestre = grafoTerrestre.Dijkstra(1)
        caminhoAereo = grafoAereo.Dijkstra(1)
        for origen, custo, destino in caminhoTerrestre:
            if destino == N:
                minTerrestre = int(custo)
                break

        for origen, custo, destino in caminhoAereo:
            if destino == N:
                minAereo = int(custo)
                break

        if(minAereo < minTerrestre):
            print(minAereo)
        else:
            print(minTerrestre)
