...
K = int(cabecalho[1]) #Capitais

#Le as Capitais
capitais = []
for i in range(K):
    capital = input().split(" ")
    capitais.append( ( int(capital[0]), int(capital[1]) ) )

#Le as cidades
cidades = []
for i in range(N-K):
    cidade = input().split(" ")
    cidades.append( ( int(cidade[0]), int(cidade[1]) ) )

#Cria as 'n' arestas ou seja toda cidade pode ir para toda cidade que não seja capital
arestasC = []
for indexC1, cidade1 in enumerate(cidades):
    for indexC2, cidade2 in enumerate(cidades):
        #não faz a distancia de uma cidade para ela mesma
        if(indexC1 < indexC2):
            #Cria a aresta
            valor = distancia(cidade1[0],cidade1[1],cidade2[0],cidade2[1])
            arestasC.append( (indexC1, indexC2, valor) )

#Cria as 'n' arestas ou seja de toda capital para todas as cidades
arestasK = []
for indexC1, capital in enumerate(capitais):
    arestasK.append(list())
    for indexC2, cidade in enumerate(cidades):
        #Cria a aresta
        valor = distancia(capital[0],capital[1],cidade[0],cidade[1])
        arestasK[indexC1].append( (indexC1 + N - K, indexC2, valor) )

#Cria um grafo sem as Capitais
grafo = Grafo()
grafo.listaAdjacencia(N-K, arestasC, direcional)

#Cria a agm deste novo grafo ( que continua sem as capitais)
grafo.kruskal()
agm = grafo.agm

#Adiciona  a menor aresta de cada Capital
for aresta in arestasK:
    #Ordena as arestas por valor
    aresta.sort(key=lambda x: x[2])

    #Adiciona a aresta mais barata a agm
    agm.inserirVertice() #Adiciona a Capital ao grafo
    agm.inserirAresta(agm.listaArestaAdjacente, aresta[0][0],aresta[0][1],aresta[0][2])
    agm.inserirAresta(agm.listaArestaAdjacente, aresta[0][1],aresta[0][0],aresta[0][2])
    agm.qtdAresta = agm.qtdAresta + 2

#Verifica quais os vertices necessários para se ligar todas as capitais
vertices = []
origen = N - K

for i in range(N - K, N):
   v = agm.pathBetween(origen, i)
   vertices.extend(v)
caminhoCapitais = list(set(vertices))

#Soma o valor de toda aresta que liga os vertices do caminho entre as capitais
valor = 0
for indice in range(N):
    if(indice in caminhoCapitais):
        for aresta in agm.listaArestaAdjacente[indice]:
            if(aresta[0] in caminhoCapitais):
                valor = valor + aresta[1]
#Como é bidirecional a aresta é somada duas vezes portanto dividisse por 2
print("{:.5f}".format(valor/2))