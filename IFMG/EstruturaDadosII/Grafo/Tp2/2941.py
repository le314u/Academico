...
"""
Ja existe ciclo? se sim tira as arestas mais baratas de modo que pare de ter ciclo
Constroi a Agm em cima da arvore pree montada
"""

direcional = True

#Entrada
N = int(input()) #Quantidade de Sedes
matrix = []
for i in range(N):
    matrix.append(list())
    linha = input()
...
for i in range(N):
    for j in range(i+1,N):
        #origen destino valor
        aresta = (i,j,matrix[i][j])
        arestas.append(aresta)
        if(matrix[j][i] == 1):
            arestasOriginais.append(aresta)

#Cria o grafo inicial
grafoOrigen = Grafo()
grafoOrigen.listaAdjacencia(N, arestasOriginais, direcional = direcional)

#Cria uma agm baseado no grafo de origem com os maiores custos entre as arestas ( ou seja o que é inviavel tirar)
grafoOrigen.kruskal(minimum = False)
preTree = grafoOrigen.agm

#Cria um grafo com todas conexoes possiveis 
grafoAll = Grafo()
grafoAll.listaAdjacencia(N, arestas, direcional = direcional)

#Cria uma Agm tendo como base o que é inviavel tirar e o que é viavel acrescentar
grafoAll.kruskal(preTree = preTree)
agm = grafoAll.agm

#Verifica qual o custo para para Construir a Rede caso não existisse nenhuma conexao
custoAgm = agm.pesoGrafo()

#Verifica o Custo de tirar o cabeamentos que não foram usados 
arestasAgm = agm.getArestas()
custoRemover = 0
for aresta in arestasOriginais:
    if(not aresta in arestasAgm):
        valorAresta = aresta[2]
        custoRemover = custoRemover + valorAresta

#Verifica o Custo de Adicionar alguma Aresta]
custoAdicionar = 0
for aresta in arestasAgm:
    if(not aresta in arestasOriginais):
        valorAresta = aresta[2]
        custoAdicionar = custoAdicionar + valorAresta

#Custo Total
print( str(custoAdicionar+custoRemover) )
