...
N = int(cabecalho[0]) #Numero de bairros
M = int(cabecalho[1]) #Numero de conexoes
P = int(cabecalho[2]) #Numero de perguntas
...
for i in range(P):
    ...
    K = int(linha[0])
    L = int(linha[1])
    perguntas.append((K,L))

#Cria um grafo
grafo = Grafo(1)
grafo.listaAdjacencia(N,arestas,direcional = direcional)
parteConexa = grafo.kruskal()

#Verifica todas as perguntas
for origen, destino in perguntas:
    #Verifica se K e L fazem parte da mesma componente conexa
    contido = False
    for componente in parteConexa:
