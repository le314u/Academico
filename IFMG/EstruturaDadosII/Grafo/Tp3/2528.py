...
class Grafo():
    ...
    def custoGrafo(self):
        custoTotal = 0
        #Soma o valor das Arestas
        for indexOrigen, linha in enumerate(self.listaArestaAdjacente):
            for destino, valor in linha:
                custoTotal = custoTotal + valor;
        #Se o Grafo for bidirecional divide por 2 pois cada aresta foi somada duas vezes
        if(self.direcional == False):
            custoTotal = custoTotal/2
        #Retona o custo do grafo
        return custoTotal
        ...
        while (len(visitation) != 0):
            ...
            for vertix, value in self.listaArestaAdjacente[currentVertice]:
                if statusVertix[vertix] == self.unvisited:
                    ...
                    visitation.append(vertix);
            statusVertix[currentVertice] = self.visited;
        #StatusVertice = Atingivel
        #Grau = Grau de cada vertice
        #Anterior = Quem é anterio ao vertice[indice]
...
while(True):
    try:
        ...
        qtdVertices = int(cabecalho[0])
        qtdArestas = int(cabecalho[1])
        ...
        #Cidades chaves
        cityKeys = input().split(" ")
        Curytyba = int(cityKeys[0])
        Riacho = int(cityKeys[1])
        Estatunido = int(cityKeys[2])
        
        #Remove as arestas referente a cidade de Estadunido
        for origen, destino, valor in arestas:
            if(origen == Estatunido or destino == Estatunido ): 
                arestas.remove( (origen,destino,valor) )

        #Cria o Grafo com as arestas Adjacentes
        grafo = Grafo(1)
        grafo.listaAdjacencia(qtdVertices, arestas, direcional=False)

        #Calcula o grau de cada vertice
        aux = grafo.grauDistancia(Curytyba)
        grau = aux[1]
        print(grau[Riacho - grafo.offset])
