...
if ( abordagem == 1):
    while True:
        try:
            #Entrada qtd Predios Qtd Vias
            cabecalho = input().split(" ")
            qtdPredios = int(cabecalho[0])
            qtdVias = int(cabecalho[1])

            #Entrada das vias
            arestas = []
            for i in range(qtdVias):
                conexao = input().split(" ")
            ...
            direcional = False
            grafo = Grafo(1)
            grafo.listaAdjacencia(qtdPredios, arestas, direcional)

            #Cria uma AGM
            parteConexa = grafo.kruskal()
            agm = grafo.agm

            #verifica a conexidade
            if( len(parteConexa)  == 1):
                #Verifica o custo total da agm
                total = agm.pesoGrafo()
