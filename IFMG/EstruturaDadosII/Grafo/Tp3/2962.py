...
#Visualizar DEBUG
def view(largura, altura, atingivel):
    for j in range(altura):
        for i in range(largura):
            if(atingivel[indiceMatrix(dimensao,i,altura-1-j)]):
                print("[ ]",end='')
            else:
                print("[X]",end='')
        print("")

#Transforma as cordenas x e y em um offSet de vetor
def indiceMatrix(dimensao, x, y):
    return ( y * dimensao[0] + x )

#Retorna os vertices que o sensor cobre
def analiseSensor(dimensao, x, y, raio):
    #Dimensao
    largura = dimensao[0]
    altura = dimensao[1]

    #ponto de origem
    inicio = (x-raio, y-raio)

    #Vetor que guarda todos os vertices
    vertices = []

    #Verifica quais os vertice inatingiveis
    for i in range(2*raio + 1):#x
        for j in range(2*raio + 1):#y
            x = inicio[0] + i
            y = inicio[1] + j

            #So é valido se não exceder a dimensao
            if( (x >= 0 and x < largura) and (y >= 0 and y < altura) ):
                #Transforma as Cordenas em um indice de Vetor
                indice = indiceMatrix(dimensao, x, y)
                vertices.append( indice )
    return vertices
...
#Entrada Dimensao do local
cabecalho = input().split(" ")
largura = int(cabecalho[0])
altura = int(cabecalho[1])
totalSensores = int(cabecalho[2])
dimensao = (largura, altura)

#Cria um vertice para cada cordenada formada por dois inteiros
vertices = []
for y in range(altura):
    for x in range(largura):
        vertices.append( indiceMatrix(dimensao, x, y))

#Entrada Sensores Cria um vetor com todos os vertices inalcançaveis
...
for i in range(totalSensores):
    ...
    y = int(sensor[1])
    raio = int(sensor[2])
    verticesSensor = analiseSensor( (largura,altura), x, y, raio)
    verticesInalcancaveis.extend(verticesSensor)

#Aux para diminuir o custo para verificar se é atingivel ou não
atingivel = []
for i in range(largura*altura):
    atingivel.append(True)
for i in verticesInalcancaveis:
    atingivel[i] = False

#Cria as 'n' Arestas andando uma unidade e ignoro os vertices inatingiveis
arestas = []
for yi in range(altura):
    for xi in range(largura):

        #Salva o vertice de origen
        origen = indiceMatrix(dimensao, xi, yi)
        xO = xi #x original
        yO = yi #y original

        #Verifica se vertice de origen é atingivel
        if(not atingivel[origen]):
            continue

        #Verifica quais os vertices atingiveis apartir do vertice de origen com 1 deslocamento
        off = [-1,0,1]
        for offH in off:#Deslocamento Oeste Leste
            for offV in off:#Deslocamento Sul Norte
                #Se for ambos igual a zero é uma aresta para o proprio vertice
                if(offH == 0 and offV == 0 ):
                    continue
                #Vertice de Destino
                x = xO + offH
                y = yO + offV
                ...
                if( (x >= 0 and x < largura) and (y >= 0 and y < altura) ):
                    #Representação em indice 
                    destino = indiceMatrix(dimensao, x, y)
                    #É atingivel?
                    if( atingivel[destino] ):
                        #Cria a aresta
                        arestas.append( (origen,destino,1) )

#Cria o Grafo com as arestas Adjacentes
direcional = False
grafo = Grafo()
grafo.listaAdjacencia(largura*altura, arestas, direcional)

porta = 0
obra = indiceMatrix(dimensao, largura-1, altura-1)
conectividadePrincipal = grafo.connectivityBetween(porta, obra)

porta = largura-1
obra = indiceMatrix(dimensao, 0, altura-1)
conectividadeSecundaria = grafo.connectivityBetween(porta, obra)

if(conectividadePrincipal or conectividadeSecundaria):
    print("S")
else:
    print("N")


#view(largura, altura, atingivel)