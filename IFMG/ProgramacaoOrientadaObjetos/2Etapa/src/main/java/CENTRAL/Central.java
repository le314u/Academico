...
import MODELO.*;
import NEWEXCEPTIONS.CPFException;
import NEWEXCEPTIONS.CriaCandidatoException;
import NEWEXCEPTIONS.FalhaNaConexaoException;
import NEWEXCEPTIONS.NomeCandidatoException;
import NEWEXCEPTIONS.NumeroCandidatoException;
import NEWEXCEPTIONS.PartidoException;
import NEWEXCEPTIONS.SiglaException;
...
public class Central {
     ...
     */
    //Funcao que gera o erro  na sigla do candidato;
    public boolean VerificaSigla(String CampoSigla)throws SiglaException{
        int cod = Estados.valueOf(CampoSigla).getCod();
        if(cod == Estados.BR.getCod()){
                  return true;
        }else {
            throw new SiglaException ("ERRO!! SIGLA INVALIDA!"); 
        }       
    }
    public void VerificaNome(int tam) throws NomeCandidatoException{
        if(tam == 0){
            
            throw new NomeCandidatoException("ERRO!!! NOME NAO PODE SER NULO!!");
        }        
    }
    public void VerificaNumero(String NumCandidato) throws NumeroCandidatoException{
       if (NumCandidato.length() != 5) {
             throw new NumeroCandidatoException("ERRO!!! NUMERO DEVE CONTER 5 DIGITOS!!");
        } else {
            int numero = 0;
            try {
                Integer.parseInt(NumCandidato);
            } catch (Exception e) {
                throw new NumeroCandidatoException("5 DIGITOS");
            }
            if (cDAO.ObjectNumero(NumCandidato) != null) {
                throw new NumeroCandidatoException( "ERRO!! "+NumCandidato+" JA FOI CADASTRADO!!");
            }
        }
       
    }
    public void VerificaCpf(String CampoCPF) throws CPFException{
        
        Documentos doc = new Documentos(CampoCPF);
        if (!doc.getValidCpf()) {
            throw new CPFException("ERRO!!! CPF INVALIDO!!!");
            
        } else {
            if (cDAO.ObjectCpf(CampoCPF) != null) {
                throw new CPFException( " ERRO!! O CPF "+ CampoCPF  +" JA FOI CADASTRADO");              
            }
        }
    }
    ...
    public boolean CadastrarCandidato(String CampoPartido, String CampoNomeCandidato, String CampoNumeroCandidato, String CampoCPF,String CampoSigla) throws Throwable {//Passa os N Campos
                            ...
                            try {
                                    presidente = VerificaSigla(CampoSigla);
                                
                            } catch (SiglaException e) {
                                //ERRO SIGLA INVALIDA
        ...
        try{
            //Verificando Nome
            VerificaNome(CampoNomeCandidato.length());
            //Verificando Numero 
            VerificaNumero(CampoNumeroCandidato);
            //verifica CPF
            VerificaCpf(CampoCPF);
        }
        catch (NomeCandidatoException e){
            JOptionPane.showMessageDialog(null,e.getMessage());
            return false;
        }
        catch(NumeroCandidatoException e){
            JOptionPane.showMessageDialog(null, e.getMessage());
            return false;
        }
...
//        }
    ...
    /*    Documentos doc = new Documentos(CampoCPF);
        ...
        } catch (CriaCandidatoException e) {
            JOptionPane.showMessageDialog(null, e.getMessage());
        }
     ...
     */
    public boolean VerificaExistenciaPArtido (String CampoNumeroPartido, String CampoNomePartido) throws PartidoException{
        Partido partido = null;
        int numeroPartido = 0;
        //Procura Partidos pelo Nome
        partido = pDAO.ObjectNome(CampoNomePartido);
        if (partido != null) {//Achou com o mesmo nome
            throw new PartidoException("ERRO!!! JA EXISTE PARTIDO COM ESTE NOME!!!!");  
        }
        ...
        if (partido != null) {//achou com mesmo numero
            throw new PartidoException("ERRO!!! JA EXISTE PARTID OCOM ESSE NOME!!");
        } else {
            return true;
        }
    ...
    public boolean CadastrarPartido(String CampoNumeroPartido, String CampoNomePartido) {
     ...
     //   int numeroPartido = 0;
      ...
      /*  try {//Tenta Converter String para numero
        ...
        try{
         verificacao = VerificaExistenciaPArtido (CampoNumeroPartido,CampoNomePartido);
