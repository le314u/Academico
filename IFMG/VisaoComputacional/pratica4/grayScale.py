...
def myGrayScale(img):
    ...
    for l in range(linhas):
        for c in range(colunas):
            newPixel = (img[l][c][0]*0.11) + (img[l][c][1]*0.59) + (img[l][c][2]*0.30)
            img[l][c][0] = newPixel #B
            img[l][c][1] = newPixel #G
            img[l][c][2] = newPixel #R
    return img
...
original = imagemRead('img.jpg')
img = imagemRead('img.jpg')
openImg = grayScale(img)
myImg = myGrayScale(img)
